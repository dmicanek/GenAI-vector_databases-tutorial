...
|---|--------|-----------|----------------|-------|
| 1 | `1. ChromaDB` | [Embeddings and a real vector store](1.%20ChromaDB/1_Creating_Embeddings_using_Chroma.ipynb), [Semantic search and RAG](1.%20ChromaDB/2_Semantic_Search_and_RAG_with_ChromaDB.ipynb), [Chunking and overlap](1.%20ChromaDB/3_Chunking_and_Overlap_with_ChromaDB.ipynb), [Hybrid search and reranking](1.%20ChromaDB/4_Hybrid_Search_and_Reranking_with_ChromaDB.ipynb) | Embedding functions, collections, metadata, `where` filters, `query()`, distances, a RAG pipeline with citations, an optional fully local variant; then chunking: the embedding model's token ceiling, a token-window splitter, chunk vectors vs. document vectors in 2D, chunk size and overlap tuned by experiment; then retrieval quality: BM25 keyword search next to dense search, Reciprocal Rank Fusion, a cross-encoder reranker, and a labelled evaluation (Recall@3, MRR@3, latency) of the four pipelines | OpenAI key (optional in nb 1, 3 and 4) |
...
- Notebook 4 of module 1 needs no key for its core: it downloads two small local models on first run (Chroma's `all-MiniLM-L6-v2`, about 80 MB, and the cross-encoder `cross-encoder/ms-marco-MiniLM-L6-v2`, about 90 MB) and runs on a CPU; `rank-bm25` provides the keyword index. Notebooks 1 to 3 do not need to be run first.
